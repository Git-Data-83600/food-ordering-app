...
router.post('/login', (request, response) => {
  ...
  // SQL statement
  const statement = `
    select id, firstName, lastName, is_active from users
    ...
    (error, users) => {
      if (error) {
        ...
        if (users.length == 0) {
          ...
          // user exists
          const { firstName, lastName, id, is_active } = users[0]
...
router.get('/all', (request, response) => {
  // SQL statement
  const statement = `
    select id, firstName, lastName, email, is_active from users
  `
...
// update the status
router.patch('/status/:userId/:status', (request, response) => {
  ...
  // execute the statement
  db.pool.execute(statement, [status, userId], (error, result) => {
    // send the result to the client
    response.send(utils.createResult(error, result))
  })
})
